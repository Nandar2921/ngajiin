    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-3 gap-4 max-w-md mx-auto mb-12">
          <div className="text-center p-4 bg-card/70 rounded-xl border border-border/60">
            <div className="text-2xl font-bold text-emerald-500">{stats.quran.toLocaleString()}</div>
            <div className="text-xs text-muted-foreground">Ayat Quran</div>
          </div>
          <div className="text-center p-4 bg-card/70 rounded-xl border border-border/60">
            <div className="text-2xl font-bold text-blue-500">{stats.hadith.toLocaleString()}</div>
            <div className="text-xs text-muted-foreground">Hadits</div>
          </div>
          <div className="text-center p-4 bg-card/70 rounded-xl border border-border/60">
            <div className="text-2xl font-bold text-purple-500">{stats.tafsir.toLocaleString()}</div>
            <div className="text-xs text-muted-foreground">Tafsir</div>
          </div>
        </div>
        ...
        <div className="max-w-md mx-auto mb-12">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-card border border-border rounded-xl focus:outline-none focus:border-emerald-500 text-foreground placeholder:text-muted-foreground/60"
            />
          </div>
        </div>
          ...
          {filteredTopics.map((topic) => {
            ...
            return (
              <button
                ...
                className="group text-left p-5 bg-card/70 border border-border/60 rounded-xl hover:border-border transition-all duration-200 hover:transform hover:scale-[1.02]"
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <ChevronRight className="w-5 h-5 text-muted-foreground/60 group-hover:text-emerald-500 transition" />
                </div>
                <h3 className="text-lg font-semibold text-foreground mb-1">{topic.name}</h3>
        ...
        {filteredTopics.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-muted-foreground">Tidak ditemukan topik "{searchTerm}"</p>
          </div>
        )}

        {/* CTA Section */}
        <div className="mt-16 text-center">
          <p className="text-muted-foreground mb-4">Tidak menemukan topik yang dicari?</p>
          <Link 
            href="/search"
